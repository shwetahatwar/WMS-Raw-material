...
interface DispatchSlipPickingListItemDao {
    ...
    @Query("SELECT COUNT (*) FROM dispatchslip_picking_list_item WHERE dispatchSlipId LIKE :dispatchSlipId AND productCode LIKE :materialCode AND batchCode LIKE :batchNumber ORDER BY timestamp ASC")
    fun getCountForBatchMaterialCode(dispatchSlipId: Int, materialCode: String, batchNumber: String): Int

    @Query("SELECT COUNT (*) FROM dispatchslip_picking_list_item WHERE dispatchSlipId LIKE :dispatchSlipId AND productCode LIKE :materialCode AND batchCode LIKE :batchNumber AND serialNumber LIKE :serialNumber ORDER BY timestamp ASC")
    fun getCountForBatchMaterialCodeSerial(dispatchSlipId: Int, materialCode: String, batchNumber: String, serialNumber: String): Int

    @Query("SELECT * FROM dispatchslip_picking_list_item WHERE dispatchSlipId LIKE :dispatchSlipId AND batchCode LIKE :batchNumber ORDER BY timestamp ASC")
    fun getItemsForBatch(dispatchSlipId: Int, batchNumber: String): LiveData<List<DispatchSlipPickingListItem>>

    @Query("SELECT * FROM dispatchslip_picking_list_item WHERE dispatchSlipId LIKE :dispatchSlipId AND productCode LIKE :materialCode AND batchCode LIKE :batchNumber ORDER BY timestamp ASC")
    ...
    @Query("SELECT * FROM dispatchslip_picking_list_item WHERE dispatchSlipId LIKE :dispatchSlipId AND submitted = 1 ORDER BY timestamp ASC")
    fun getSubmitedDispatchDetails(dispatchSlipId: Int): LiveData<List<DispatchSlipPickingListItem>>

    @Query("UPDATE dispatchslip_picking_list_item SET submitted = 1 WHERE dispatchSlipId LIKE :dipsatchSlipId")
    suspend fun updateSubmittedStatus(dipsatchSlipId: String)

    @Query("DELETE from dispatchslip_picking_list_item WHERE dispatchSlipId LIKE :dipsatchSlipId")
